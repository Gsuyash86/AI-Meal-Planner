...
export default function RecipesPage() {
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <button 
            ...
            onClick={() => document.getElementById('add-recipe-form')?.scrollIntoView({ behavior: 'smooth' })}
          >
      ...
      <section className="mb-12">
        {loading ? (
          ...
          <div className="text-center py-12 bg-dark-card rounded-xl">
            <ChefHat className="w-12 h-12 mx-auto text-cred-cyan mb-4" />
            <h3 className="text-xl font-semibold mb-2">No recipes yet</h3>
            <p className="text-text-secondary mb-4">Add your first recipe to get started</p>
            <button 
              className="btn btn-primary"
              onClick={() => document.getElementById('add-recipe-form')?.scrollIntoView({ behavior: 'smooth' })}
            >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {recipes.map((recipe) => (
              <div 
                key={recipe._id} 
                className="card p-5 hover:shadow-lg transition-shadow cursor-pointer h-full flex flex-col"
              ...
              >
                {recipe.imageUrl ? (
                  ...
                  <div className="aspect-video bg-dark-card rounded-lg mb-4 flex items-center justify-center">
                    <ChefHat className="w-8 h-8 text-text-tertiary" />
                  </div>
                )}
                <h3 className="text-lg font-medium mb-2 line-clamp-1">{recipe.title}</h3>
                {recipe.description && (
                  <p className="text-text-secondary text-sm mb-3 line-clamp-2 flex-grow">
                    {recipe.description}
                  </p>
                )}
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2 text-xs text-text-tertiary">
                    ...
                    {recipe.difficulty && (
                      <span className="px-2 py-0.5 bg-dark-card rounded-full">
                        {recipe.difficulty}
                      </span>
                    )}
                  </div>
      ...
      <section id="add-recipe-form" className="mb-12">
